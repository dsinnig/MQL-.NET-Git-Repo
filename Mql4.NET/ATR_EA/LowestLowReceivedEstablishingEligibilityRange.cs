...
namespace biiuse
{
    ...
    public override void update()
    {
        ...
        if (barCounter > context.getLengthIn1MBarsOfWaitingPeriod() + 1)
        {
            ...
            context.addLogEntry("Range established at: " + mql4.IntegerToString(rangePips) + " microp pips. HH=" + mql4.DoubleToString(rangeHigh, mql4.Digits) + ", LL=" + mql4.DoubleToString(rangeLow, mql4.Digits), true);
            ...
            else
            {
                ...
                if (((rangeHigh - rangeLow) < ((context.getPercentageOfATRForMaxVolatility() / 100.00) * context.getATR())) &&
                     (mql4.Ask > rangeLow + context.getATR() * (context.getPercentageOfATRForMaxRisk() / 100.00) + buffer))
                {
                    ...
                    context.addLogEntry("Range (" + mql4.IntegerToString(rangePips) + " micro pips) is greater than max risk (" + mql4.DoubleToString(context.getPercentageOfATRForMaxRisk(), 2) +
                                        "%) but less than max. volatility (" + mql4.DoubleToString(context.getPercentageOfATRForMaxVolatility(), 2) + "%) of ATR (" + mql4.IntegerToString(ATRPips) + " micro pips). Ask price is greater than entry price.", true);
                     ...
                     (mql4.Ask < rangeLow + context.getATR() * (context.getPercentageOfATRForMaxRisk() / 100.00) + buffer))
                {
                    ...
                    //write to log
                    context.addLogEntry("Range (" + mql4.IntegerToString(rangePips) + " micro pips) is greater than max risk " + mql4.DoubleToString(context.getPercentageOfATRForMaxRisk(), 2) +
                                       "% but less than max. volatility " + mql4.DoubleToString(context.getPercentageOfATRForMaxVolatility(), 2) + "% of ATR (" + mql4.IntegerToString(ATRPips) + " micro pips). Ask price is less than entry price.", true);
